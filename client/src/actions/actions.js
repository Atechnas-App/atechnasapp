...
export function addMercadoPago(payload) {
  return async function (dispatch) {
    const auth = await axios(
      `https://auth.mercadopago.com.ar/authorization?client_id=${APP_ID}&response_type=code&platform_id=mp&state=RANDOM_ID&redirect_uri=https://localhost:3001/accesmp`
    );
    dispatch({
      type: FILTER,
      payload: auth.data,
    ...
    try {
      await firebase
        ...
        .then(({ user }) => {
          ...
          dispatch(startLoding())(dispatch(loginStore()));
...
export function postJobs(id, payload) {
  return async function () {
    const newJob = await axios.post(
      "http://localhost:3001/api/newProfile/" + id,
      payload
    );
    return newJob;
  };
